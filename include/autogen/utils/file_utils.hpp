...
#endif
#include <vector>
#define AUTOGEN_MAX_EXE_PATH_LEN 4096
...
struct FileUtils {
  ...
  static inline std::vector<std::string> prefixes{
      {AUTOGEN_HOME "/", AUTOGEN_HOME "/data/", "../data/", "../../data/",
       "../../../data/", "../../../../data/", "./autogen/", "./autogen/data/",
  ...
  static bool find_file(const std::string& org_file_name,
    ...
    for (int j = 0; j < 2; j++) {
      for (std::size_t i = 0; !f && i < prefixes.size(); i++) {
        ...
        if (f) {
          relative_file_name =
              fs::canonical(fs::path(relative_file_name)).u8string();
          file_found = true;
          break;
        }
      }
      char exe_path[AUTOGEN_MAX_EXE_PATH_LEN];
      int l = get_exe_path(exe_path, AUTOGEN_MAX_EXE_PATH_LEN);
      if (l) {
        char path_to_exe[AUTOGEN_MAX_EXE_PATH_LEN];
        int exe_name_pos =
            extract_path(exe_path, path_to_exe, AUTOGEN_MAX_EXE_PATH_LEN);
        // if (exe_name_pos) {
        //  root = path_to_exe;
        //}
      }
