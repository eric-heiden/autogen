...
struct CudaFunctionSourceGen {
  ...
  void emit_kernel(std::ostringstream &code, const std::ostringstream &body,
    ...
    if (!is_function) {
      ...
      code << "    printf(\"ERROR: thread index %i in function \\\"" << function_name << "\\\" exceeded provided "
  ...
  void emit_send_functions(std::ostringstream &code) const {
    ...
    fprintf(stderr, "Error %i (%s) in function \")" << function_name << R"(\" while sending thread-local input data to GPU: %s.\n",
    ...
    code << "  cudaError status = cudaMemcpy(dev_" << function_name
         ...
         << " * sizeof(Float), "
            "cudaMemcpyHostToDevice);\n";
    code << R"(  if (status != cudaSuccess) {
    fprintf(stderr, "Error %i (%s) in function \")" << function_name << R"(\" while sending global input data to GPU: %s.\n",
            status, cudaGetErrorName(status), cudaGetErrorString(status));
    return false;
  }
)";
  ...
  if (status != cudaSuccess) {
    fprintf(stderr, "Error %i (%s) in function \")" << function_name << R"(\" while executing CUDA kernel: %s.\n",
            status, cudaGetErrorName(status), cudaGetErrorString(status));
    exit((int)status);
  }
  ...
  if (status != cudaSuccess) {
    fprintf(stderr, "Error %i (%s) in function \")" << function_name << R"(\" while retrieving output from kernel: %s.\n",
            status, cudaGetErrorName(status), cudaGetErrorString(status));
    exit((int)status);
  })";
...
class CudaModelSourceGen : public CppAD::cg::ModelCSourceGen<Base> {
 ...
 public:
  ...
  std::string forward_one_source() {
    ...
    // if (this->_loopTapes.empty()) {
      // dep = this->_fun.Forward(1, indVars);
      dep = this->_fun.Forward(0, indVars);
    // } else {
    //   /**
    //    * Contains loops
